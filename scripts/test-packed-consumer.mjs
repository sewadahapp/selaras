...
async function inspectSsr(prefixed = true, explicitTheme = false) {
  ...
  try {
    ...
    assert.ok(css.includes('--selaras-resolved-color-published-fill:var(--selaras-color-published-fill,#123456)'), 'generated CSS must contain the registered default')
    assert.ok(css.includes('--selaras-resolved-color-seeded-fill:var(--selaras-color-seeded-fill,#fd5e53)'), 'generated CSS must preserve the seed as the resting fill')
    ...
    if (process.env.SELARAS_PACKED_BROWSER) {
      ...
      try {
        ...
        await page.goto(url)
        await page.locator('#packed-responsive').waitFor({ state: 'hidden' })
        ...
        await page.locator('#packed-responsive').waitFor({ state: 'visible' })
        await page.locator('[data-testid="remaining-date-field"] button').last().click()
        await hasRecipe('[data-testid="remaining-date-heading"]', 'tracking-widest')
...
try {
  ...
  for (const publicName of ['useCommandPalette', 'useDrawer', 'useIcons', 'useLocale', 'useMessages', 'useModal', 'useRippleEnabled', 'useSlideover', 'createTableColumnHelper', 'useToast'])
    assert.match(generatedImports, new RegExp(`\\b${publicName}\\b`), `${publicName} must remain a public Nuxt auto-import`)
  for (const internalName of ['provideFormField', 'useFormField', 'useIsMobile', 'useTable', 'useComboboxSelect', 'flattenItems', 'isOptionGroup'])
  ...
  writeFileSync(appPath, readFileSync(appPath, 'utf8')
    .replace('tw:hidden tw:tablet:block', 'hidden laptop:block')

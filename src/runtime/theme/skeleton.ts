...
export const skeletonTheme = tv({
  slots: {
    // --selaras-resolved-surface-elevated (neutral-50) reads as barely-off-white against the
    ...
    // --selaras-resolved-border-default is the same "visible neutral block on white" token
    // Slider's own track already uses for the identical reason.
    base: 'block rounded-[var(--selaras-resolved-radius-md)] bg-[var(--selaras-resolved-border-default)]',
  },
  variants: {
    animation: {
      ...
      shimmer: {
        base: 'bg-[linear-gradient(90deg,var(--selaras-resolved-border-default)_25%,var(--selaras-resolved-border-hover)_50%,var(--selaras-resolved-border-default)_75%)] bg-[length:200%_100%] animate-[selaras-skeleton-shimmer_1.6s_ease-in-out_infinite]',
      },
    },
  },
  defaultVariants: {

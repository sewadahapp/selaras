...
export const modalTheme = tv({
  ...
  variants: {
    ...
    // The animation classes live entirely in this variant rather than the
    // base slot strings above - tailwind-merge doesn't know `animate-in`/
    // `animate-none` conflict (they're `tw-animate-css` utilities, not
    // core Tailwind, so they're outside its built-in conflict map), so
    // adding an `animate-none` override on top of an already-present
    // `animate-in` in the base string does NOT replace it - both classes
    // would just coexist in the final string. Keeping the "on" state's
    // classes here too (not just "off") sidesteps that entirely: only one
    // branch's classes are ever present at once, nothing to conflict-
    // resolve at all.
    transition: {
      true: {

...
function matches(option: ResolvedItemOption) {
  return !(props.listbox || (props.creatable && props.multiple)) || !props.searchable || contains(option.label, props.searchText)
}
const visibleOptions = computed(() => props.flatOptions.filter(matches))
const createText = computed(() => {
  if (!props.creatable || !props.multiple || props.forceSelection)
    return ''
  const text = props.searchText.trim()
  const lower = text.toLowerCase()
  if (!text || props.flatOptions.some(option => String(option.value).toLowerCase() === lower || option.label.toLowerCase() === lower)
    || props.selectedValues?.some(value => typeof value === 'string' && value.toLowerCase() === lower)) {
    return ''
  }
  return text
})
...
function onItemSelect(event: Event, value: string | number) {
  ...
  // chip should not use Reka's multiple-listbox toggle to remove it.
...
<template>
  ...
  <div v-if="flatOptions.length === 0 && !createText" v-bind="emptyProps">
    <slot name="empty">
      {{ messages.noOptions }}
    </slot>
  </div>
  <component :is="primitives.viewport" v-else :id="listbox ? listboxId : undefined" v-bind="viewportProps">
    <div v-if="listbox && visibleOptions.length === 0 && !createText" v-bind="emptyProps">
      <slot name="empty-filter">
        {{ messages.noResultsFound }}
      </slot>
    </div>
    <div v-else-if="creatable && multiple && visibleOptions.length === 0 && !createText" v-bind="emptyProps">
      <slot name="empty-filter">
        {{ messages.noResultsFound }}
      </slot>
    </div>
    <ComboboxEmpty v-else-if="!listbox && !(creatable && multiple)" v-bind="emptyProps">
      <slot name="empty-filter">
        {{ messages.noResultsFound }}
      </slot>
    </ComboboxEmpty>
    ...
    <template v-else>
      <component :is="primitives.item" v-if="createText" :value="createText" data-selaras-create-option v-bind="itemProps">
        {{ messages.createOption(createText) }}
      </component>
      <template v-for="(entry, index) in items" :key="index">
        ...
        <component
          ...
          v-bind="itemProps"
          @select="onItemSelect($event, toOption(entry).value)"
        >

...
/**
 * Merges a consumer's app.config.selaras.icons override onto the default
 * (Hugeicons) registry - mirrors useComponentTheme's selaras.ui.<key> merge (see
...
export function useIcons(): ComputedRef<IconRegistry> {
  const appConfig = useAppConfig() as { selaras?: { icons?: Partial<IconRegistry> } }
  return computed(() => ({ ...defaultIcons, ...appConfig.selaras?.icons }))
}

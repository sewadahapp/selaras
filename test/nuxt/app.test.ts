...
describe('app', () => {
  ...
  it('emits light/dark runtime role overrides into an SSR-safe style layer', async () => {
    ...
    const wrapper = await mountSuspended(App)
    await flush()
    const style = [...document.head.querySelectorAll('style')].find(node => node.textContent?.includes('--selaras-color-premium-fill'))
    expect(style?.textContent).toContain('--selaras-color-premium-fill: #5134a8;')
    expect(style?.textContent).toContain('--selaras-color-premium-fill: #a78bfa;')

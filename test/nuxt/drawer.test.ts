...
describe('drawer', () => {
  ...
  it('modal="false" does not hide the rest of the page from assistive tech, unlike the modal default', async () => {
    wrapper = await mountSuspended(Drawer, { props: { open: true, title: 'Non-modal', description: 'Non-modal', modal: false } })
  ...
  it('is modal (hides the rest of the page) by default', async () => {
    wrapper = await mountSuspended(Drawer, { props: { open: true, title: 'Modal', description: 'Modal' } })
  ...
  it('renders no handle when handle is false', async () => {
    wrapper = await mountSuspended(Drawer, { props: { open: true, title: 'Filters', description: 'Filters', handle: false } })
  ...
  it('renders no close button when close is false', async () => {
    wrapper = await mountSuspended(Drawer, { props: { open: true, title: 'Filters', description: 'Filters', close: false } })
  ...
  it('still emits escapeKeyDown when dismissible is false, but does not close', async () => {
    wrapper = await mountSuspended(Drawer, { props: { open: true, title: 'Filters', description: 'Filters', dismissible: false } })
  ...
  it('renders the content slot in place of header/body/footer entirely, but still registers title/description for a11y', async () => {
    wrapper = await mountSuspended(Drawer, {
      props: { open: true, title: 'Custom panel', description: 'A panel with fully custom content' },
    ...
    expect(document.getElementById(titleId!)?.textContent).toBe('Custom panel')
  })

  it('replaces the close icon via the close-icon slot', async () => {
    wrapper = await mountSuspended(Drawer, {
      props: { open: true, title: 'Filters', description: 'Filters' },
  ...
  it.each([
    ...
    wrapper = await mountSuspended(Drawer, { props: { open: true, title: 'Filters', description: 'Filters', side } })
  ...
  it('renders no overlay element when overlay is false', async () => {
    const withOverlay = await mountSuspended(Drawer, { props: { open: true, title: 'A', description: 'A' } })
    expect(document.body.querySelector('[data-state="open"].bg-black\\/50')).toBeTruthy()
    withOverlay.unmount()

    wrapper = await mountSuspended(Drawer, { props: { open: true, title: 'B', description: 'B', overlay: false } })
  ...
  it('strips the animation classes entirely when transition is false', async () => {
    const withTransition = await mountSuspended(Drawer, { props: { open: true, title: 'A', description: 'A' } })
    expect(document.body.querySelector('[role=dialog]')!.className).toContain('data-[state=open]:animate-in')
    withTransition.unmount()

    wrapper = await mountSuspended(Drawer, { props: { open: true, title: 'No transition', description: 'No transition', transition: false } })
    const dialog = document.body.querySelector('[role=dialog]')!
  ...
  it('emits afterLeave when the closing animation finishes, not the opening one', async () => {
    wrapper = await mountSuspended(Drawer, { props: { open: true, title: 'A', description: 'A' } })
  ...
  it('emits afterLeave immediately on close when transition is off, with no animation to wait for', async () => {
    wrapper = await mountSuspended(Drawer, { props: { open: true, title: 'A', description: 'A', transition: false } })
  ...
  it('passes snapPoints through to the underlying DrawerRoot', async () => {
    wrapper = await mountSuspended(Drawer, { props: { open: true, title: 'A', description: 'A', snapPoints: [0.3, 0.6, 1] } })

...
test('renders explicit semantic modes before JavaScript or hydration', async ({ browser, page, goto }) => {
  ...
  try {
    ...
    await expect(serverRenderedPage.locator('#inner-primary-read')).toHaveCSS('color', 'rgb(40, 50, 60)')
...
test('isolates explicit modes, inherited partial tokens and body portals through hydration and updates', async ({ page, goto }) => {
  ...
  await expect(page.locator('#outer-primary-read')).toHaveCSS('color', 'rgb(220, 221, 222)')
  await expect(inner).toHaveCSS('background-color', 'rgb(40, 50, 60)')
  await expect(page.locator('#inner-primary-read')).toHaveCSS('color', 'rgb(40, 50, 60)')
  ...
  await expect(page.locator('#inner-primary-read')).toHaveCSS('color', 'rgb(41, 51, 61)')
  await expect(page.locator('#portal-primary-read')).toHaveCSS('color', 'rgb(41, 51, 61)')
  await page.locator('#toggle-scope').evaluate(element => (element as HTMLButtonElement).click())

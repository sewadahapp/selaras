...
test('renders RadioGroup semantic ring, indicator and card leaves', async ({ page, goto }) => {
  ...
  for (const dark of [false, true]) {
    ...
    for (const role of ['primary', 'enterprise']) {
      ...
      await expect(checked).toHaveClass(/data-\[state=checked\]:ring-\[var\(--_selaras-color-fill\)\]/)
      await expect(checked.locator('[data-state="checked"]').first()).toHaveClass(/bg-\[var\(--_selaras-color-fill\)\]/)

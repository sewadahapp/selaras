...
describe('built-in semantic defaults', () => {
  it('covers every role in both modes without reading remapped legacy color variables', () => {
    ...
    expect(registry.primary.light.fill).toBe('var(--color-primary-500)')
    expect(registry.warning.light.onFill).toBe('var(--color-neutral-950)')
    expect(registry.neutral.light.fill).toBe('var(--color-neutral-950)')
    expect(registry.neutral.dark.fill).toBe('var(--color-neutral-50)')
  ...
  it('uses the consumer foundation prefix in all modes and preserves opaque dark subtle surfaces', () => {
    ...
    expect(registry.primary.light.fill).toBe('var(--tw-color-primary-500)')
    expect(registry.primary.dark.subtle).toBe('color-mix(in oklab, var(--tw-color-primary-500) 24%, var(--ui-bg))')
    expect(registry.neutral.dark.border).toBe('var(--tw-color-neutral-800)')
  })
})
